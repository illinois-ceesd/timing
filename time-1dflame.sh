...
then 
    DRIVER_MD5SUM=$(md5sum ./flame1d.py | cut -d " " -f 1)
else
...
EOF
    ...
    *)
        printf "Host: Unknown\n"
        PYOPENCL_TEST=port:pthread python -m mpi4py ./flame1d.py -i timing_params.yaml
        ;;
esac
...
if [[ -f "${RUN_LOG_FILE}" ]]; then
    ...
    SUMMARY_FILE_NAME="log_${timestamp}.sqlite"
    ...
    cp ${SUMMARY_FILE_NAME} timing/flame_logs
    cd timing
    git add flame_logs/${SUMMARY_FILE_NAME}
