...
if [[ -f "${RUN_LOG_FILE}" ]]; then
    ...
    STARTUP_TIME=`sqlite3 ${RUN_LOG_FILE} 'select SUM(value) from t_step WHERE step BETWEEN 0 and 0;'`
    FIRST_10_STEPS=`sqlite3 ${RUN_LOG_FILE} 'select SUM(value) from t_step WHERE step BETWEEN 0 and 9;'`
    SECOND_10_STEPS=`sqlite3 ${RUN_LOG_FILE} 'select SUM(value) from t_step WHERE step BETWEEN 10 and 19;'`
